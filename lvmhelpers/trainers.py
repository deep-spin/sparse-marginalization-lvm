...
class Trainer:
    ...
    Some common configuration (checkpointing frequency,
    ...
    def __init__(
        ...
        """
        :param game: A nn.Module that implements forward();
            it is expected that forward returns a tuple of (loss, d),
            where loss is differentiable loss to be minimized and d
            is a dictionary (potentially empty) with auxiliary
        ...
        :param validation_data: A DataLoader for the validation set
            (can be None)
        :param device: A torch.device on which to tensors should be stored
        :param callbacks: A list of egg.core.Callback objects that
            can encapsulate monitoring or checkpointing
        """
        self.game = game
        ...
        # NB: some optimizers pre-allocate buffers
        # before actually doing any steps
        # since model is placed on GPU within Trainer,
        # this leads to having optimizer's state and model parameters
        # on different devices. Here, we protect from that
        # by moving optimizer's internal state to the proper device
